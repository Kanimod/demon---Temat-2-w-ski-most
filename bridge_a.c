...
// Funkcja wyświetla aktualny stan symulacji.
/* Format: A-x y>>> [>> id kierunek >>] <<<z w-B
  gdzie:
   x - liczba aut po stronie A
   y - liczba aut czekających po stronie A
   id - numer samochodu przejeżdżającego
   z - liczba aut oczekujących po stronie B
   w - liczba aut po stronie B
*/
...
void enterBridgeA(int id)
{
   ...
   // Jeśli most jest zajęty, samochód zwalnia mutex, czeka chwilę i próbuje ponownie
    while(bridge == 1)
    {
...
void enterBridgeB(int id)
{
    ...
    // Oczekiwanie aż most stanie się wolny
    while(bridge == 1)
    {
        pthread_mutex_unlock(&mutex);
        usleep(1000);
